...
qC_ind <- q %>%
  mutate(q_in = (((q_cfs / (4.0424*10^10))*3600)*12)*24 )
```
...
```{r}
T_c <- rbind(Turbidity_1, Turbidity_2, Turbidity_3, Turbidity_C)
```
...
watershed_c <- st_read('Control.geojson')%>%
  dplyr::filter(id=='globalwatershed')
